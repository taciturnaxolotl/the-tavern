export default function barChartGenerator(
    ...
    const longestDataPoint =
        Math.max(...data.map((data) => data.toString().length)) + 2
    ...
    for (let j = 0; j < data.length; j++) {
        barChartString += `${
            ('(' + data[j] + ')')
                .toString()
                .padEnd(columnWidth * 2 - data[j].toString().length, ' ') + ' '
        } `
    }
    ...
    barChartString = ''
    for (let j = 0; j < data.length; j++) {
        barChartString += `${labels[j].padEnd(
            columnWidth * 2 - labels[j].toString().length,
            ' '
        )}  `
    }
    barChartStringArray.push(barChartString)
    ...
    for (let i = 0; i < height + 1; i++) {
        ...
        for (let j = 0; j < data.length; j++) {
            if (Math.round(data[j] / divisionFactor) >= i) {
                barChartString +=
                    dataPoint.padEnd(columnWidth - dataPoint.length, padChar) +
                    ' '
            } else {
                barChartString +=
                    padChar.padEnd(columnWidth - dataPoint.length, padChar) +
                    ' '
            }
        }
